...
uint32_t color_timer 	= 0;		// counter for COLOR_INTERVAL 
uint8_t  spectrum_part 	= 0;		// switch for current light spectrum 
uint16_t color_idx 		= 0;		// current color can be between 0 - 255 
uint8_t curr_color_granularity	= 1;// granularity, is amount of color change

// Accelerometer
Adafruit_TinyLSM303 	lsm;			
int32_t storedVector 			= 0;	// last motionVector 
int32_t newVector 				= 0;	// motion Vector 	
uint8_t nr_shackes				= 0;	// counter for shackes 
uint32_t last_shacke			= 0;	// time since last shacke

void setup() {
	pixels.begin();
	brightness = getMaxBrightness(brightness);
...
byte getMicLevel() {
	...
	level = getMaxBrightness(level);			// use max brightness to dimm
	return level;
}

/*	map brightness between 0 and MAX_BRIGHTNESS to save energy */
byte getMaxBrightness(byte brightness) {
